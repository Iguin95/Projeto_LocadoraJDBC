	...
	public List<ClienteFilme> encontrarClienteComFilme(String cpf) {
		...
		try {
			...
			/*Declaração e inicialização de um Map para associar os CPFs dos clientes
			 (String) aos objetos ClienteFilme. Isso ajuda a agrupar filmes 
			  para cada cliente.*/
            ...
            while (rs.next()) { //Enquanto existir uma linha do ResultSet, faça...
                ...
                String clienteCpf = rs.getString("cliente.cpf"); //Obtém o CPF do cliente da linha atual do ResultSet e... 
                ClienteFilme clienteFilme = clienteFilmeMap.get(clienteCpf); //...tenta encontrar um ClienteFilme associado a esse CPF no Map.

                /*Se não houver um 'ClienteFilme' associado ao CPF (ou seja, o cliente ainda não foi processado):
                 - Cria um novo objeto Cliente.
                 - Define o nome e o CPF do cliente com os valores do ResultSet.
                 - Cria um novo objeto 'ClienteFilme'.
                 - Define o cliente no objeto 'ClienteFilme'.
                 - Adiciona o 'ClienteFilme' ao Map, associando-o ao CPF do cliente.*/
		...
		/*OBS: Essa estrutura permite agrupar facilmente todos os filmes de cada cliente, evitando duplicações 
		 * e facilitando a construção da lista final de ClienteFilme: 
		 - Se um cliente já existe no Map (com o mesmo CPF), seus filmes são adicionados à lista de filmes desse cliente.
		 - Se um cliente não existe no Map, um novo objeto ClienteFilme é criado e adicionado ao Map.*/
	}
